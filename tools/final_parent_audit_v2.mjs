...
try {
  for(const vp of viewports){
    ...
    const url=`${BASE}?v=parent-audit-v2-${Date.now()}-${vp.width}`;
    ...
    const initial=await page.evaluate((expectedContainers)=>{
      ...
      return {
        ...
        actionTexts:Array.from(document.querySelectorAll('#view-home button')).map(b=>(b.textContent||'').trim()).filter(Boolean)
      };
    },expectedContainers);
    ...
    for(const d of expectedDates){
      assert(initial.activeSlots.filter(s=>s.date===d).length===6,`${vp.name}: ${d} non ha 6 slot attivi`);
    }
    ...
    if(await firstAction.count()){
      ...
      const modalMetrics=await page.evaluate(()=>{
        ...
        return {
          modalWidth:boxRect?.width||0,
          viewport:window.innerWidth,
          scrollHeight:box?.scrollHeight||0,
          clientHeight:box?.clientHeight||0,
          canScroll:box ? box.scrollHeight>=box.clientHeight : false,
      ...
      assert(modalMetrics.fieldFonts.every(n=>n>=16),`${vp.name}: almeno un campo mobile ha font <16px (rischio zoom iOS): ${modalMetrics.fieldFonts.join(',')}`);
      ...
      const pickupState=await page.evaluate(()=>({
        pickupHidden:document.getElementById('mini-pickup-name')?.classList.contains('hidden'),
        noticeHidden:document.getElementById('mini-exit-notice')?.classList.contains('hidden')
      }));
    ...
    // Generazione PDF/e-mail simulata: la chiamata Apps Script e il QR sono intercettati, nessun dato viene scritto.
    if(vp.width===390){
    ...
    reports.push({viewport:vp,initial,modal:modalMetrics??null,manage:manageMetrics,pageErrors,criticalRequestFailures});
    await context.close();
  }
  ...
  if(failures.length){
    console.error('FINAL_PARENT_AUDIT_FAILURES='+JSON.stringify(failures));
    process.exitCode=2;
  } else {
    console.log('FINAL_PARENT_AUDIT_OK');
  }
} finally {
  await browser.close();
}
