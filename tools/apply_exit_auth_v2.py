...
# --- Core booking / PDF / email ---
p = Path('ministage-complete.js')
s = p.read_text(encoding='utf-8')

s = s.replace("const VERSION = '2026.09-clean-monolith-v1';", "const VERSION = '2026.09-exit-authorization-v2';")
s = s.replace('<option value="autonoma">Può uscire autonomamente</option>', '<option value="autonoma">Autorizzo l\'uscita autonoma al termine del MiniStage</option>')
s = s.replace('<option value="ritiro_adulto">Deve attendere un adulto</option>', '<option value="ritiro_adulto">Ritiro da parte di un adulto incaricato</option>')

old = '''      <input id="mini-pickup-name" type="text" placeholder="Adulto incaricato del ritiro (se previsto)" class="w-full p-2.5 rounded-lg border border-indigo-100 bg-white text-xs hidden">
...
new = '''      <input id="mini-pickup-name" type="text" placeholder="Adulto incaricato del ritiro (se previsto)" class="w-full p-2.5 rounded-lg border border-indigo-100 bg-white text-xs hidden">
...
if old not in s:
    raise SystemExit('Blocco form autorizzazione non trovato')
s = s.replace(old, new, 1)

old = "if (!d.exitMode || !d.parentGuardianName || !d.parentGuardianRole || !d.declarationAccepted) return 'Completa la sezione relativa all’uscita e alla dichiarazione.';"
new = "if (!d.exitMode || !d.parentGuardianName || !d.parentGuardianRole) return 'Completa la sezione relativa all’uscita.';\n    if (!d.declarationAccepted) return d.exitMode === 'autonoma' ? 'Per proseguire devi spuntare l’autorizzazione esplicita all’uscita autonoma.' : 'Per proseguire devi confermare la modalità di uscita selezionata.';"
if old not in s:
    raise SystemExit('Validazione autorizzazione non trovata')
s = s.replace(old, new, 1)

old = """        declarationAccepted: true,
        declarationTimestamp: timestamp,
        declarationVersion: 'MiniStage-2026-v1',
        authorizationPaperReceived: false"""
new = """        declarationAccepted: true,
        declarationTimestamp: timestamp,
        ...
        authorizationPaperReceived: false"""
if old not in s:
    raise SystemExit('Dati autorizzazione non trovati')
s = s.replace(old, new, 1)

old = """      const text = `Il/La sottoscritto/a ${res.parentGuardianName || '________________'}, in qualità di ${res.parentGuardianRole || '________________'}, dichiara di aver indicato nel modulo di prenotazione l’uscita autonoma dello/a studente/ssa ${res.nome || '________________'} al termine del MiniStage presso l’IIS Primo Levi di Seregno.`;
...
new = """      const text = `Il/La sottoscritto/a ${res.parentGuardianName || '________________'}, in qualità di ${res.parentGuardianRole || '________________'}, AUTORIZZA il/la proprio/a figlio/a ${res.nome || '________________'} a lasciare autonomamente l’IIS Primo Levi di Seregno al termine del MiniStage indicato nella prenotazione.`;
      pdf.text(pdf.splitTextToSize(text,180),15,38);
      pdf.setFont('Helvetica','bold'); pdf.text('Codice prenotazione:',15,72); pdf.setFont('Courier','bold'); pdf.text(res.code,58,72);
...
if old not in s:
    raise SystemExit('Seconda pagina PDF non trovata')
s = s.replace(old, new, 1)

old = "let message = `La prenotazione ${res.code} è confermata. Classe assegnata: ${res.classeAssegnata || 'da definire'}.`;"
new = "let message = `La prenotazione ${res.code} è confermata. Classe assegnata: ${res.classeAssegnata || 'da definire'}.${res.exitMode === 'autonoma' ? ' Il PDF allegato contiene come seconda pagina l’autorizzazione all’uscita autonoma: stamparla, firmarla e consegnarla alla scuola.' : ''}`;"
if old not in s:
    raise SystemExit('Messaggio email non trovato')
s = s.replace(old, new, 1)

old = '''      extra.push(`<div><span class="text-gray-400 font-medium">Uscita:</span> <strong class="text-indigo-950">${res.exitMode === 'autonoma' ? 'Autonoma - consegnare modulo firmato' : res.exitMode === 'ritiro_adulto' ? `Attesa adulto${res.pickupAdultName ? ` (${esc(res.pickupAdultName)})` : ''}` : 'Non indicata'}</strong></div>`);
...
new = '''      extra.push(`<div><span class="text-gray-400 font-medium">Uscita:</span> <strong class="text-indigo-950">${res.exitMode === 'autonoma' ? 'Autonoma - autorizzata online' : res.exitMode === 'ritiro_adulto' ? `Attesa adulto${res.pickupAdultName ? ` (${esc(res.pickupAdultName)})` : ''}` : 'Non indicata'}</strong></div>`);
...
if old not in s:
    raise SystemExit('Dettagli ricevuta non trovati')
s = s.replace(old, new, 1)

marker = '  async function promoteNext(slot) {'
insert = '''  async function downloadMiniStageBookingPdf(code) {
    try {
...
'''
if marker not in s:
    raise SystemExit('Punto download PDF non trovato')
s = s.replace(marker, insert + marker, 1)

old = '    window.downloadWaitlistPdf=downloadWaitlistPdf;\n    window.editMiniStageSlot=editMiniStageSlot;'
new = '    window.downloadWaitlistPdf=downloadWaitlistPdf;\n    window.downloadMiniStageBookingPdf=downloadMiniStageBookingPdf;\n    window.editMiniStageSlot=editMiniStageSlot;'
if old not in s:
    raise SystemExit('Expose download PDF non trovato')
s = s.replace(old, new, 1)
p.write_text(s, encoding='utf-8')

# --- Teacher console ---
p = Path('ministage-admin-console-v2.js')
s = p.read_text(encoding='utf-8')
s = s.replace("const VERSION = '2026.09-admin-console-v2';", "const VERSION = '2026.09-admin-console-exit-auth-v2';")

active_re = re.compile(r"  function renderActiveList\(\) \{.*?\n  \}\n\n  function waitPosition", re.S)
active_new = '''  function renderActiveList() {
...
s, n = active_re.subn(active_new, s, count=1)
if n != 1:
    raise SystemExit('renderActiveList non sostituito')

auth_re = re.compile(r"  function renderAuth\(\) \{.*?\n  \}\n\n  function renderScannerSessions", re.S)
auth_new = '''  function renderAuth() {
...
s, n = auth_re.subn(auth_new, s, count=1)
if n != 1:
    raise SystemExit('renderAuth non sostituito')
p.write_text(s, encoding='utf-8')

# --- Cache bust loader ---
p = Path('index.html')
s = p.read_text(encoding='utf-8')
s = s.replace('ministage-complete.js?v=20260906b', 'ministage-complete.js?v=20260906c')
s = s.replace('ministage-admin-console-v2.js?v=20260906b', 'ministage-admin-console-v2.js?v=20260906c')
p.write_text(s, encoding='utf-8')

# Static assertions
core = Path('ministage-complete.js').read_text(encoding='utf-8')
admin = Path('ministage-admin-console-v2.js').read_text(encoding='utf-8')
html = Path('index.html').read_text(encoding='utf-8')
for token in ['Autorizzo espressamente mio/a figlio/a', 'seconda pagina del PDF ricevuto via e-mail', 'exitAuthorizationAccepted', 'authorizationPaperRequired', 'AUTORIZZA il/la proprio/a figlio/a', 'downloadMiniStageBookingPdf']:
    assert token in core, token
for token in ['Autorizzazione online acquisita', 'Modulo firmato da consegnare', 'PDF + autorizzazione', 'Genitore/tutore']:
    assert token in admin, token
assert '20260906c' in html
print('EXIT_AUTH_PATCH_OK')
